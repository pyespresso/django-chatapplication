...
from infra.utils.http_error import BadRequest
...
class RequestValidation:
    ...
    def __call__(self, request):
        ...
        '''
        request_info = {
            'route_name': request.url_info['view_name'],
            'url_parameters': request.url_info['kwargs'],
            'query_parameters': dict(request.GET),
            'request_body': request._json_body if request._is_body_json else None,
...
class ValidateParamType(object):
    ...
    def __validate_string_params(self):
        '''This method is used to validate whether the param value is a valid string.
...
#    request_config = cache.get('request_configs').get(request_info['route_name'])[request_info['method']]
    request_config = cache.get('request_configs', {}).get(request_info['route_name'], {}).get(request_info['method'], {})
    if request_config:
        response = {}
        ...
        if request_body_schema:
            data = request_info['request_body']
